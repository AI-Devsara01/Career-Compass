    ...
    if request.method == 'POST':
        ...
        try:
            ...
            with open(filepath, "rb") as doc_file:
                ...
                text = result.value
            filepath = newpath  # Continue using .docx version
        elif filename.lower().endswith(".wps"):
            try:
                # convert .wps to .docx using libreoffice (if installed)
                subprocess.run(["soffice", "--headless", "--convert-to", "docx", filepath, "--outdir", app.config["UPLOAD_FOLDER"]])
