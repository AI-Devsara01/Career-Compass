...
# Slugs for roles (used for URLs or filenames etc.)
ROLE_SLUGS = {
...
# Grade resume sections
def grade_sections(resume_text):
    section_scores = {}
    for section in SECTION_HEADERS:
        pattern = re.compile(rf"{section}", re.IGNORECASE)
        section_scores[section] = SECTION_WEIGHTS[section] if pattern.search(resume_text) else 0
...
# Recommend role based on keyword match
def recommend_role(resume_text):
    ...
    recommendations = {
        skill: SKILL_RESOURCES[skill] for skill in missing_keywords if skill in SKILL_RESOURCES
    }
    ...
    return {
        "role_slug": ROLE_SLUGS.get(best_match),
