...
# Example SKILL_RESOURCES dictionary (can be customized further)
SKILL_RESOURCES = {
    ...
    # Add more skills and their resources as needed
}

# Extract text from resume file (PDF, DOCX, DOC) using PyMuPDF (fitz) for PDF
def extract_text(file_path):
    ...
    print(f"Extracted text from resume: {text[:500]}")  # Show first 500 characters for debugging
...
def recommend_role(resume_text):
    ...
    print(f"Recommendations: {recommendations}")  # Debugging line to show the resources
...
def main():
    ...
    if resume_text:
        ...
        print(f"\nRecommended Role: {result['recommended_role']}")
        print(f"ATS Score: {result['ats_score']}")
        print(f"Matched Keywords: {result['matched_keywords']}")
        print(f"Missing Keywords: {result['missing_keywords']}")
        print(f"Recommendations for Missing Skills: {result['recommendations']}")
    else:
